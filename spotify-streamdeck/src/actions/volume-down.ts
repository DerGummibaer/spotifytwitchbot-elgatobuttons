...
@action({ UUID: "com.leon.spotifycontrol.voldown" })
export class VolumeDownAction extends SingletonAction<VolumeStepSettings> {
  ...
  override async onKeyDown(ev: KeyDownEvent<VolumeStepSettings>): Promise<void> {
    ...
    try {
      ...
      streamDeck.logger.error("vol_down failed", err);
      await ev.action.showAlert();
    }
  }
}
